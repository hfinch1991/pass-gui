...
chrome.storage.session.get("passphrase", (data) => {
  if (data.passphrase) {
    ...
    chrome.runtime.sendMessage({ action: "check-passphrase" }, (response) => {
      statusEl.innerText = "";
...
function doUnlock() {
  ...
  chrome.runtime.sendMessage({ action: "unlock", passphrase: pw }, (response) => {
    unlockBtn.disabled = false;
    unlockBtn.textContent = "Unlock";

    if (response && response.status === "ok") {
      cachedPassphrase = pw;
      chrome.storage.session.set({ passphrase: pw });
...
function doSearch(term) {
  ...
  chrome.runtime.sendMessage({ action: "search", domain: term, passphrase: cachedPassphrase }, (response) => {
    statusEl.innerText = "";
...
function fetchAndFill(path) {
  ...
  chrome.runtime.sendMessage({ action: "fetch", entry: path, passphrase: cachedPassphrase }, (response) => {
    if (response && response.error) {
