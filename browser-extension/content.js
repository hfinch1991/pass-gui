// content.js - Robust Positioning & Zoom Support
...
function addIconToInput(input) {
  ...
  // Use fixed positioning for better zoom/scroll stability
  ...
  const updatePosition = () => {
    // If input is no longer in DOM or hidden, hide host
  ...
  // 1. Sync with browser zoom and viewport changes
  if (window.visualViewport) {
    window.visualViewport.addEventListener('resize', updatePosition);
    window.visualViewport.addEventListener('scroll', updatePosition);
  }
  
  // 2. Sync with window events
  window.addEventListener('resize', updatePosition);
  window.addEventListener('scroll', updatePosition, { passive: true });

  // 3. Sync with dynamic layout changes
...
function showMenu(input) {
  ...
  // Menu also uses fixed positioning to stay with the input during zoom
...
function saveCurrent(input, menuHost) {
  ...
  if (!path) return;
  chrome.runtime.sendMessage({ action: "save", path, username: user, password: pass, url: window.location.href }, (res) => {
